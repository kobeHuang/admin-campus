  ...
  <div class="sidebar">
    ...
    <el-menu router :default-active="$route.path" background-color="#2A3F54" text-color="#fff" active-text-color="#ffd04b"
     :collapse-transition="false" :collapse="isCollapse" unique-opened>
      <template v-for="(route, index) in $router.options.routes" v-if="route.name != 'login' && route.name != '404'">
        <!-- 二级导航 -->
        <el-submenu :index="route.path" :key="index" v-if="route.children" :class="{'isSmall': isCollapse }">
          <template slot="title">
            ...
            <el-menu-item :index="r.path" v-for="r in route.children" :key="r.path">{{r.name}}</el-menu-item>
  ...
  computed: {
    targetRoute() {
      let targetRoute = this.$router.options.routes.find(route => {
        return this.$route.matched[0].regex.test(route.path);
      });
      return targetRoute;
    },
    sideBarVisible() {
